...
from app.config import UPLOAD_DIR, QWEN_MODEL
...
app = FastAPI(title="AI Conference Helper")

templates = Jinja2Templates(directory=str(Path(__file__).parent / "templates"))
app.mount("/static", StaticFiles(directory=str(Path(__file__).parent / "static")), name="static")

ALLOWED_EXTENSIONS = {".mp3", ".m4a", ".wav", ".flac", ".ogg", ".mp4", ".webm", ".aac"}
...
async def meeting_delete(meeting_id: int):
    delete_meeting(meeting_id)
    return RedirectResponse(url="/", status_code=303)


@app.post("/meeting/{meeting_id}/reanalyze")
async def reanalyze(background_tasks: BackgroundTasks, meeting_id: int, custom_prompt: str = Form(None)):
    meeting = get_meeting(meeting_id)
